define(function(require, exports, module) {
	...
	var template=require("./assets/checkoutroomtrack.tpl");
	...
	function setTableData(model,aw,widget,memberShipCardNo){
		...
		aw.ajax({
			url : "api/report/checkoutroomtrack",
			type : "POST",
			data : {
				"s":search,
				"status":status,
	...
	var checkoutroomtrack = ElView.extend({
		attrs:{
    		template:template,
    		model : {}
        },
				...
				model:{
					...
					items : [{
						...
						handler:function(){ 
							...
							var start = widget.get("subnav").getValue("createDate").start;
							var end = widget.get("subnav").getValue("createDate").end;
							var URL = "api/report/checkoutroomtracktoexcel?s="+s+"&status="+processStatus+"&schedulType=Secretary" +
							"&start="+start+"&end="+end;
	...
	module.exports = checkoutroomtrack;
});
