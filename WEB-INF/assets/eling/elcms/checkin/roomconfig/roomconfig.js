			...
			return {
				...
				params:function(){
					...
					return {
						...
						"checkInRoomConfing.status":widget.get("subnav").getValue("status"),
					...
					columns:[{
						...
						formatparams:[{
							...
							show:function(data,row){
								if(row.status.key=="Doing" && row.checkInRoomConfing.status.key=="Pending"){
									return true;
								}else{
									return false;
								}
		...
		initCard:function(widget){
			return {
				...
				saveaction:function(){
					...
					params.individuDemandConfirm=$(".J-individuDemandConfirm").prop("checked");
					params.beddingConfirm=$(".J-beddingConfirm").prop("checked");
					params.fitmentPosiConfirm=$(".J-fitmentPosiConfirm").prop("checked");
					params.keyNumberConfirm=$(".J-keyNumberConfirm").prop("checked");
					params.description=widget.get("card").getValue("description");
					params.disposeDate=moment().valueOf();
					params.disposeUser=store.get("user").pkUser;
					if(params.individuDemandConfirm && params.beddingConfirm && params.fitmentPosiConfirm && params.keyNumberConfirm){
						params.status="Pended";
					}else{
						params.status="Pending";
					}
					widget.save("api/checkinroomconfig/save",aw.customParam(params),function(data){
						widget.get("list").refresh({
				...
				model:{
					...
					items:[{
						...
						label:"个性化需求",
						format:function(){
							return '<div class="col-md-7 format_line">'+
									...
									'	<input name="individuDemandConfirm" type="checkbox" class="J-individuDemandConfirm J-individuDemandConfirm-true">'+
									'</label></div>';
						}
					},{
						name:"bedding",
						...
						format:function(){
							return '<div class="col-md-7 format_line">'+
									...
									'<div class="col-md-1 format_line"><label class="checkbox-inline">' +
									'	<input name="beddingConfirm" type="checkbox" class="J-beddingConfirm J-beddingConfirm-true">'+
									'</label></div>';
						}
					},{
						name:"fitmentPosi",
						...
						format:function(){
							return '<div class="col-md-7 format_line">'+
									...
									'<div class="col-md-1 format_line"><label class="checkbox-inline">' +
									'	<input name="fitmentPosiConfirm" type="checkbox" class="J-fitmentPosiConfirm J-fitmentPosiConfirm-true">'+
									'</label></div>';
						}
					},{
						name:"keyNumber",
						...
						format:function(){
							return '<div class="col-md-7 format_line">'+
									...
									'<div class="col-md-1 format_line"><label class="checkbox-inline">' +
									'	<input name="keyNumberConfirm" type="checkbox" class="J-keyNumberConfirm J-keyNumberConfirm-true">'+
									'</label></div>';
						}
					},{
						name:"description",
