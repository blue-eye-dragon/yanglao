define(function(require, exports, module) {
	...
	var Naturalgasandle = BaseView.extend({
					...
					search : function(str) {
						...
						aw.ajax({
							...
							data:{
								...
								fetchProperties:"*,status.value,memberSigning.checkInDate,memberSigning.members.personalInfo.name,memberSigning.room.number," +
								"checkInNaturalGas.*,checkInNaturalGas.disposeUser.*,checkInNaturalGas.acceptanceUser.*"
							},
							dataType:"json",
			...
			return {
				...
				params:function(){
					...
					return {
						...
						"checkInNaturalGas.status":subnav.getValue("handle"),
						fetchProperties:"*,status.value,memberSigning.checkInDate,memberSigning.members.personalInfo.name,memberSigning.room.number," +
							"checkInNaturalGas.*,checkInNaturalGas.disposeUser.*,checkInNaturalGas.acceptanceUser.*"
					};
				},
				...
				model:{
					columns:[{
						...
						key:"checkInNaturalGas.disposeUser.name",
						name:"处理人"
					},{
						key:"checkInNaturalGas.disposeDate",
						name:"处理时间",
						format:"date"
					},{
						key:"status.value",
						...
						formatparams:[{
							...
							show:function(data,row){
								if(row.status.key=="Doing" && row.checkInNaturalGas.status.key=="Pending"){
									return true;
								}else{
									return false;
								}
					...
					head:{
						buttons:[{
							...
							handler:function(){
								...
								if(!params.naturalGas){
									Dialog.alert({
										content:"没有要处理的燃气开通"
									});
									return false;
								}
								params.naturalGasConfirm=true;
								params.disposeDate=moment().valueOf();
								params.disposeUser=store.get("user").pkUser;
								params.naturalGasMoney=this.getEditData()[0].money;
								params.status="Pended";
								aw.saveOrUpdate("api/checkinnaturalgas/save",aw.customParam(params), function(data){
									Dialog.mask(false);
									widget.list2Card(false);
									widget.get("subnav").hide(["return"]).show(["handle","time","checkInStatus","search"]);
									widget.get("list").refresh({
			...
			aw.ajax({
				...
				success:function(data){
					...
					if(data.naturalGas){
						pushData.push({
							name:"天然气",
							money:data.naturalGasMoney==null?0:data.naturalGasMoney
						});
					}
	...
	module.exports = Naturalgasandle;
});