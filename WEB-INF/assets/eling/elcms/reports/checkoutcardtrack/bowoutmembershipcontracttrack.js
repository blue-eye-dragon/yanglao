define(function(require, exports, module) {
	...
	var template=require("./assets/bowoutmembershipcontracttrack.tpl"); 
	...
	function setTableData(model,aw,widget,memberShipCardNo){
		...
		var start = widget.get("subnav").getValue("applyDate").start;
		var end = widget.get("subnav").getValue("applyDate").end;
		aw.ajax({
			url : "api/report/bowoutmembershipcontracttrack",
			type : "POST",
			data : {
				"s":search,
				"status":status,
	...
	var bowoutmembershipcontracttrack = ElView.extend({
		attrs:{
    		template:template,
    		model : {}
        },
				...
				model:{
					...
					items : [{
						...
						handler:function(){ 
							...
							var start = widget.get("subnav").getValue("applyDate").start;
							var end = widget.get("subnav").getValue("applyDate").end;
							var URL = "api/report/bowoutmembershipcontracttracktoexcel?s="+s+"&status="+processStatus+"&schedulType=Secretary" +
							"&start="+start+"&end="+end;
		...
		afterInitComponent:function(params,widget){
			...
			if (params){				
				...
				time.end = params.end;
				widget.get("subnav").setValue("applyDate", time);
	...
	module.exports = bowoutmembershipcontracttrack;
});
