					...
					items:[{
						...
						handler:function(){
							...
							for(var i=0;i<datas.length;i++){
								params+=datas[i].pkAccompanyHospitalize+",";
							}
							if(datas.length!=0){
								Dialog.confirm({
									...
									confirm:function(){
										aw.ajax({
											url : "api/accompanyhospitalizemanager/batchsubmitapplication",
											type : "POST",
											data : {
			...
			var grid=new Grid({
				...
				params:function(){
					...
					return {
						...
						fetchProperties:"*,flowStatus.key,flowStatus.value,member.personalInfo.name,handoverPerson.name,handoverPerson.pkUser," +
				...
				model:{
					...
					columns:[{
						...
						name:"状态",
						format:function(value,row){
							if(value.key=="Commited"){
								return "已提交";
							}else if(value.key=="Temporary"){
								return "button";
							}else{
								return value.value;
							}
						...
						formatparams:[{
							...
							handler:function(index,data,rowEL){
								aw.ajax({
									url : "api/accompanyhospitalizemanager/submit",
									type : "POST",
									data : {
