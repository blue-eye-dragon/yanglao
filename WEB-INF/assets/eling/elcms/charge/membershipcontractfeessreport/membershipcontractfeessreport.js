...
define(function(require, exports, module) {
    ...
    var params = null;
    var times = 0;
	var ContractFeeMonthReport = BaseView.extend({
		events:{
		...
		initSubnav:function(widget){
		    ...
		    times=0;
			var min=moment().format("YYYY年MM月"+i18ns.get("charge_shipfees_money","卡费")+"月报");
			var years=[];
			...
			var buttonGroup=[{
				   ...
				   handler:function(key,element){
					   ...
					   var title=moment(monthFirstDay).format("YYYY年MM月卡费月报");
				   ...
				   handler:function(key,element){
					   ...
					   var month = widget.get("subnav").getValue("month");
					   var monthFirstDay= year + "-" +month;
					   var title=moment(monthFirstDay).format("YYYY年MM月"+i18ns.get("charge_shipfees_money","卡费")+"月报");
					   widget.get("subnav").setTitle(title);
				   }
			   },{
		   		    id:"chargeStatus",
...
//					}
					return {
						...
						"endDate":moment(monthFirstDay).valueOf(),
						"endDateEnd":moment(monthLastDay).valueOf(),
						"start":(chargeStatus == "UnCharge" || chargeStatus == "")? moment(monthFirstDay).valueOf():"",
						"end":(chargeStatus == "UnCharge" || chargeStatus == "")? moment(monthLastDay).valueOf():"",
				...
				model:{
					columns:[{
						...
						name:"收费状态",
						format:function(row,value){
							if(value.chargeStatus.key == "UnCharge"){
								return "未收费";
							}else if(value.chargeStatus.key =="Charged"){
								return "已收费";
							}else if(value.chargeStatus.key == "Receiving"){
								return "已到账";
							}
						}
					}]
		...
		afterInitComponent:function(widget){
			if(params){
				...
				var title=moment(params.year+"-"+params.month).format("YYYY年MM月"+i18ns.get("charge_shipfees_money","卡费")+"月报");
	...
	module.exports = ContractFeeMonthReport;
});
