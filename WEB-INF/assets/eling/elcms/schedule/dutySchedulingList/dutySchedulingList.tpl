<div class="el-modole-container el-dutySchedulingList">
	<div class='J-subnav'></div>
	<div class='J-form hidden'></div>
	<div class='J-gridform hidden'></div>
	<div class='J-remarkgridform hidden'></div>
	...
	<h3 class='J-component-span' style="margin: 10px 0; display: inline-block;">老年公寓排班表</h3>
	</div>
								...
								<div>
									<table style='border:0 none;margin-bottom:0;' class='data-table-column-filter table table-bordered table-striped'>
										<tbody class="J-grid-table" style="text-align:center">
			    							...
			    							{{#each this.data}}
				    							{{#each this.userdutylist}}
												<tr data-index = "{{@index}}" index="{{../this.index}}">
													{{#if this.build}}
													<td rowspan="{{../../this.userdutylist.length}}" style="vertical-align:middle;">{{this.build.name}}</td>
													{{/if}}
