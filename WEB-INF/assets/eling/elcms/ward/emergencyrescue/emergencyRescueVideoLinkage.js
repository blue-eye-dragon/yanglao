							...
							handler:function(index,data,rowEle){
								...
								aw.ajax({
									...
									success : function(result) {
										...
										var URL = location.protocol+'//'+location.host+localStorage.getItem("ctx")+'html/monitors.jsp';//URL，不包含参数部分
										if(pkPlace) {
											newWinUrl(URL+'?pkPlace='+pkPlace,'紧急求助视频联动');
											//window.open(URL+'?pkPlace='+pkPlace,'紧急求助视频联动页');
										} else if (coordinate) {
											newWinUrl(URL+'?coordinate'+coordinate,'紧急求助视频联动');
											//window.open(URL+'?coordinate'+coordinate,'紧急求助视频联动页');
										} else {
											Dialog.alert({
				       							content : "找不到相关监控!"
				       						 });
